										...
										<div class="flex-grow-1 px-30 flex-grow-1 bg-img dask-bg bg-none-md" style="background-position: right bottom; background-size: auto 100%; background-image: url({{asset('ed_admin/images/svg-icon/color-svg/custom-1.svg')}})">
											<div class="row">
												<div class="col-12 col-xl-7">
													<h3 class="fw-bolder">Heading Match Sub Question</h3>
												</div>
											</div>
										</div>
									</div>
								...
								<div class="row">
									<div class="col-xl-10 col-lg-10 col-md-10 col-sm-12 col-xs-12 mx-auto">
                                        <div>
                                            ...
                                            <form action="{{route('admin.settings.mock.heading-match.store.sub-question')}}" method="POST">
                                                @csrf
                                                <input type="hidden" name="question_id" value="{{$question_id}}">
                                                <div class="form-group">
                                                    <label for="name" class="mb-2">Text</label>
                                                    <input type="text" class="form-control" name="text" placeholder="Write heading questing text">
                                                </div>
                                                <div class="form-group">
                                                    <label for="name" class="mb-2">Question Answer</label>
                                                    <select name="blank_answer" id="" class="form-control">
                                                        <option value="">Plesase select a answer</option>
                                                        @foreach ($mockHeadingTitle as $rows)
                                                            <option value="{{$rows->id}}">{{$rows->heading_title}}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                                <div class="form-group">
                                            ...
                                            <tbody>
                                                @foreach ($mockHeadingMatch as $items)
                                                    <tr>
                                                        ...
                                                        <td>{{$items->marks}}</td>
                                                        <td>
                                                            <a href="{{route('admin.settings.mock.heading-match.delete.sub-question', ['id'=>$items->id])}}" class="btn btn-danger btn-sm">Delete</a>
                                                        </td>
                                                    </tr>
                                                @endforeach
                                            </tbody>
